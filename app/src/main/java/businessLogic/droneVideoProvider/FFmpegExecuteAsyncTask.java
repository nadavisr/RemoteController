...
/*        catch (TimeoutException e)
        {
            Log.e("FFmpegLocal timed out", e.getMessage());
            return new CommandResult(false, e.getMessage());
        }*/
        catch (Exception e)
        {
          Log.e("Error running FFmpegLocal", e.getMessage());
        }
        finally
        {
    ...
    private void checkAndUpdateProcess() throws TimeoutException, InterruptedException
    {
        while (!Util.isProcessCompleted(process)) {
            ...
            if (timeout != Long.MAX_VALUE && System.currentTimeMillis() > startTime + timeout) {
                throw new TimeoutException("FFmpegLocal timed out");
            }
